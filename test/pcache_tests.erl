...
error_logger:error_msg("Emptying cache ~p~n", [Cache]),
    pcache:empty(Cache),
error_logger:error_msg("Unregister ~p~n", [tc_pcache]),
whereis(tc_pcache)  =:= undefined orelse begin unlink(tc_pcache), unregister(tc_pcache) end,

error_logger:error_msg("Unregister ~p~n", [tc2_pcache]),
    whereis(tc2_pcache) =:= undefined orelse begin unlink(tc2_pcache), unregister(tc2_pcache) end,
error_logger:error_msg("Exiting cache ~p~n", [Cache]),
    exit(Cache, normal).
